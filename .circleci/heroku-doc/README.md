...
git config --global user.name "Cent Pour Cent Dom"
git config --global user.email centpourcentdom@gmail.com
...
* To retrieve the GIT COMMIT ID from the latest docker image : `docker inspect --format '{{ index .Config.Labels "git.commit.id"}}' "quay.io/domzone/une_proposition:stable-latest"`
