...
secrethub org init -f --name="${SECRETHUB_ORG}" --description="The secrethub org to manage secrets for anything related to https://github.com/Jean-Baptiste-Lasselle/siteweb_dom.git"
secrethub repo init "${SECRETHUB_ORG}/${SECRETHUB_REPO}"

# --- #
# create a service account
secrethub service init "${SECRETHUB_ORG}/${SECRETHUB_REPO}" --description "Circle CI  Service Account for the [domzone-cicd] Cirlce CI context for the https://github.com/Jean-Baptiste-Lasselle/siteweb_dom.git Circle CI Pipeline" --permission read | tee ./.the-created.service.token
...
# --- #
# finally, in Circle CI, you created a 'domzone-cicd' context in the [https://github.com/1718-io] Github Org
# and in that 'domzone-cicd' Circle CI context, you set the 'SECRETHUB_CREDENTIAL' env. var. with
...
* For the https://ric-carl.herokuapp.com/ website, the container image definition will be stored at `quay.io/domzone/une_proposition`
...
export QUAY_USERNAME="domzone+carl"
